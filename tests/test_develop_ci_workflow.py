...
class DevelopCiWorkflowTests(unittest.TestCase):
    ...
    def test_package_is_gated_and_artifact_is_identifiable(self) -> None:
        ...
        self.assertIn('"Version: 0.1.7"', self.workflow)
        self.assertRegex(
            self.workflow,
            r"name: DouK-Manager_Windows_X64-v0\.1\.7-\$\{\{ steps\.identity\.outputs\.ref \}\}-run-\$\{\{ github\.run_number \}\}-\$\{\{ steps\.identity\.outputs\.sha \}\}",
        )
