...
def _reflow_task(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    columns.addWidget(_card(content, "任务配置", (form,)), 5)
    columns.addWidget(_card(content, "预览与执行结果", (output,)), 6)
    ...
    root.addWidget(
        _card(
            ...
            "任务操作",
            (actions,),
            note="主操作仍直接调用 V0.1.6 原按钮与信号槽；这里只重新组织视觉层级。",
        )
    )
    _restore(root, tail)
...
def _reflow_audit(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    top = QHBoxLayout()
    top.setSpacing(14)
    top.addWidget(_card(content, "审计参数", (controls,)), 5)
    top.addWidget(_card(content, "审计状态", (notice, summary, progress, warnings)), 7)
    root.addLayout(top)
    ...
    bottom.addWidget(_card(content, "选中账号详情", (details,)), 4)
    bottom.addWidget(
        _card(
            content,
            "决定与应用",
            (decisions, actions, output),
            note="所有决定仍使用原模型和确认流程，不会因界面重排自动写入主档。",
        ),
        6,
    )
...
def _reflow_batch(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    columns = QHBoxLayout()
    columns.setSpacing(14)
    columns.addWidget(_card(content, "批次参数", (form, actions)), 5)
    columns.addWidget(_card(content, "生成结果", (output,)), 7)
    root.addLayout(columns, 1)
    _restore(root, tail)
...
def _reflow_queue(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    columns = QHBoxLayout()
    columns.setSpacing(14)
    columns.addWidget(_card(content, "任务模板与顺序", (task_area,)), 7)
    columns.addWidget(_card(content, "本次运行策略", (options,)), 4)
    root.addLayout(columns, 1)
...
def _reflow_collector(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    columns = QHBoxLayout()
    columns.setSpacing(14)
    columns.addWidget(_card(content, "采集服务与正式路径", (intro, paths, actions)), 5)
    columns.addWidget(_card(content, "服务输出", (output,)), 7)
    root.addLayout(columns, 1)
    _restore(root, tail)
...
def _reflow_post(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    columns = QHBoxLayout()
    columns.setSpacing(14)
    columns.addWidget(_card(content, "路径与维护操作", (paths, actions)), 5)
    columns.addWidget(_card(content, "维护结果", (output,)), 7)
    root.addLayout(columns, 1)
    _restore(root, tail)
...
def _reflow_settings(page: QWidget, root: QLayout, items: list[QLayoutItem]) -> None:
    ...
    config.addWidget(_card(content, "正式路径", (paths,)), 6)
    config.addWidget(_card(content, "任务默认值", (defaults, note)), 4)
    root.addLayout(config)

    action_layout = QHBoxLayout()
    action_layout.addStretch(1)
    if save_paths.widget() is not None:
        action_layout.addWidget(save_paths.widget())
    if save_all.widget() is not None:
        action_layout.addWidget(save_all.widget())
    root.addWidget(
        _card(
            content,
            "保存与重新验证",
            (_layout_item(action_layout),),
            note="路径修复与全部设置保存仍执行 V0.1.6 原来的安全检查流程。",
        )
    )

    maintenance = QHBoxLayout()
    maintenance.setSpacing(14)
    maintenance.addWidget(_card(content, "下载引擎更新", (update,)), 4)
    maintenance.addWidget(_card(content, "历史引擎回退", (rollback,)), 7)
